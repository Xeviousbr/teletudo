...
class Adm
{
    ...
    public function EnderNovo() {
        /* $qry = DB::table('logradouro')
            ->select('pessoa.Nome','tplogradouro.nometplog','logradouro.NomeLog','endereco.Numero')
        ...
        $qry = DB::table('logradouro')
            ->select('pessoa.Nome','pessoa.Cep','pessoa.email',
    ...
    public function qUser() {
        $sql = "SELECT Count(0) as Quant FROM pessoa Where updated_at > DATE_SUB(now(),INTERVAL 1 DAY) ";
        $qry = DB::select( DB::raw($sql));
        return $qry[0]->Quant;
    }
