...
// Criar uma classe só para a PlayDelivery
// Se possível colocar ela dentro de uma pasta, integracoes


/* Cancelar Entrega
É possível cancelar uma Entrega criada em dois momentos: Quando o sistema está procurando Entregadores ou quando um Entregador já aceitou uma Entrega mas ainda não chegou ao ponto inicial. Depois que o Entregador finalizar o primeiro ponto, uma entrega não poderá mais ser cancelada. */
    ...
    */
    ...
    public function CriaRegistro($idPedido, $idCliente, $Teste, $tpEnt) {
        ...
        if ($Teste==0) {
            ...
            $sql = "insert into entrega (idEntregadora, idPedido, idCliente, HoraIN) values ( ".
                ...
                "now())";
            DB::update($sql);
            $Cons = DB::table('entrega')->select('id')->where('idPedido', '=', $idPedido)->first();
            return $Cons->id;
        } else {
            return 1;
        }
    ...
    private function DadosCliente ($idCliente) {
        $ConsP = DB::table('pessoa')
            ...
            ->join('estado', 'estado.ID', '=', 'cidade.Estado_ID')
            ->join('pais', 'pais.ID', '=', 'estado.idPais')
            ->join('cep', 'cep.NrCep', '=', 'pessoa.CEP')
            ->select('pessoa.Nome','pessoa.fone'
                ,'endereco.Numero','endereco.Complemento',
                'logradouro.NomeLog',
                'tplogradouro.nometplog',
                ...
                'pais.Nome as NomePais',
                'cep.lat','cep.lon')
            ->where('pessoa.ID', '=', $idCliente)
            ->first();
        return $ConsP;
    }

    public function PedeOrcamento($idPedido, $Teste, $TpEntrega) {
        ...
        switch ($TpEntrega) {
            case 0:
                ...
                $ret = $this->PedeOrgPlay($qryE, $ConsF, $ConsC, $Teste);
                if ($ret>0) {$Tpe=0;}
                break;
            case 1:
                // Entrega pelo fornecedor, por distância
                $this->loga('POR DISTÂNCIA');
                $ret = $this->OrcamentoFornD($qryE, $ConsF);
                if ($ret>0) {$Tpe=1;}
                break;
            case 2:
    ...
    private function PedeOrgPlay($qryE, $ConsF, $ConsP, $Teste) {
        $cPlay = new PlayDelivery();
        $ret = $cPlay->OrcamentoPlay($qryE, $ConsF, $ConsP, $Teste);
        $this->setVlrEntrega($cPlay->getVlrEntrega(), "OrcamentoPlay");
    ...
    private function OrcamentoFornD($qryE, $ConsF) {
        $ret=0;
    ...
    public function getVlrNosso() {
        if ($this->VlrNosso==-1) {
            ...
            $Cons = DB::table('TConfigVlrEntrega')
                ...
                ->where('Limite','<', $this->getVlrEntrega())
                ->orWhere('id', '1')
                ->orderBy('Valor', 'desc')
                ->first();
    ...
    public function VeQtoFornRecebeu($id) {
        ...
        $FornecRecebeu+=$this->getVlrEntrega();
    ...
    private function EfetivaPedidoNoBD($idPedido) {
        ...
        $idEntregadora = 1;
        $ContaMov->credita_entregadora($idEntregadora, $this->getVlrEntrega());

        // ACUMULAR VALOR DEVIDO AO FORNECEDOR
        $compras=$this->getCompras($idPedido);
        $ContaMov->DescobreFornecedorPelosItens($idPedido);

        $captador=$ContaMov->credita_fornecedor($compras+$this->getVlrNosso()+$this->getVlrEntrega(),
            $this->getVlrEntrega()+$this->getVlrNosso());
        ...
        $Valor=$compras + $this->getVlrEntrega() + $this->getVlrNosso();
    ...
    public function getValorTotal($idPedido) {
        if ($this->VlrTotal==0) {
            ...
            // Valor da entrega
            $vEntrega=$this->getVlrEntrega();

            // Nosso valor
            $vNosso=$this->getVlrNosso();
    ...
    public function getVlrEntrega() {
        if ($this->VlrEntrega==0) {
            $this->loga('getVlrEntrega:VlrEntrega = 0');
            $this->VlrEntrega = Session::get('VlrEntrega');
            $this->loga("Session::get('VlrEntrega') = ".Session::get('VlrEntrega'));
            if ($this->VlrEntrega==0) {
                $this->loga('VlrEntrega = 0');
                if ($this->idPedido==0) {
                    $this->loga('this->idPedido =0');
                    $this->loga("Session::get('PED') = ".Session::get('PED'));
                    $this->idPedido = Session::get('PED');
                    if ($this->idPedido==0) {
